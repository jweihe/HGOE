...
class HCL(nn.Module):
    ...
    def calc_loss_g(x, x_aug, temperature=0.2):
        ...
        # x_abs = x.norm(dim=1)
        # x_aug_abs = x_aug.norm(dim=1)
        x_abs = x.norm(dim=1) + 1e-8  # 防止模为零
        x_aug_abs = x_aug.norm(dim=1) + 1e-8
        ...
        pos_sim = sim_matrix[range(batch_size), range(batch_size)]
        loss_0_denominator = sim_matrix.sum(dim=0) - pos_sim

        loss_1_denominator = sim_matrix.sum(dim=1) - pos_sim

        loss_0 = pos_sim / loss_0_denominator
        loss_1 = pos_sim / loss_1_denominator
